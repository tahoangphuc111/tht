...
class WikiFlowTests(TestCase):
    ...
    def test_non_author_cannot_edit_article(self):
        ...
        response = self.client.get(
            reverse('wiki:article-edit', args=[self.article.pk])
        )
    ...
    def test_regular_user_can_comment_on_article(self):
        ...
        captcha_answer = get_resp.wsgi_request.session.get(
            'captcha_answer', 1
        )
    ...
    def test_article_list_can_sort_by_vote_score(self):
        ...
        ArticleVote.objects.create(
            user=self.other_user, article=other_article, value=1
        )
