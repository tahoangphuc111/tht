...
from .models import (
    Article, Category, Choice, CodingExercise,
    CodingTestCase, Comment, Profile, Question, UploadedFile,
)
...
class SignUpForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        placeholders = {
            "username": "Tên đăng nhập", "first_name": "Tên",
            "last_name": "Họ", "email": "Email",
            "password1": "Mật khẩu", "password2": "Nhập lại mật khẩu",
        }
...
class ProfileForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["display_name"].widget.attrs.update({"class": "form-control", "placeholder": "Tên hiển thị"})
...
class CategoryForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["name"].widget.attrs.update({"class": "form-control", "placeholder": "Ví dụ: Thuật toán đồ thị"})
        self.fields["slug"].widget.attrs.update({"class": "form-control", "placeholder": "thuat-toan-do-thi"})
...
class ArticleForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["slug"].widget.attrs.update({"class": "form-control", "placeholder": "segment-tree-co-ban"})
...
class CommentForm(forms.ModelForm):
    content = MartorFormField(widget=MartorWidget(attrs={"class": "form-control", "placeholder": "Đặt câu hỏi, góp ý lời giải, hoặc bổ sung mẹo tối ưu..."}))
...
class QuestionForm(forms.ModelForm):
    question_file = forms.FileField(label="Upload nội dung từ file", required=False, help_text="Có thể upload PDF/DOCX/TXT. Nếu để trống nội dung, hệ thống sẽ lấy text từ file.", widget=forms.FileInput(attrs={"class": "form-control", "accept": ".pdf,.docx,.txt"}))
    content = MartorFormField(label="Nội dung câu hỏi", required=False, widget=MartorWidget(attrs={"class": "form-control", "placeholder": "Nội dung câu hỏi (hỗ trợ Markdown)..."}))
    explanation = MartorFormField(label="Giải thích đáp án", required=False, widget=MartorWidget(attrs={"class": "form-control", "placeholder": "Giải thích đáp án (sẽ hiển thị sau khi người dùng trả lời)..."}))

    class Meta:
        model = Question
        fields = ("content", "question_file", "explanation", "order")
        widgets = {"order": forms.NumberInput(attrs={"class": "form-control", "placeholder": "Thứ tự ưu tiên hiển thị"})}
    ...
    def clean(self):
        ...
        if not content and upload:
            try:
                ...
                self.add_error("question_file", f"Không thể đọc file này: {error}")
                return cleaned_data
            if extracted_text:
                cleaned_data["content"] = extracted_text
            else:
                self.add_error("question_file", "Không trích xuất được nội dung từ file này.")
        elif not content:
...
class CodingExerciseForm(forms.ModelForm):
    allowed_languages = forms.MultipleChoiceField(choices=(), required=False, widget=forms.CheckboxSelectMultiple)
    checker_language = forms.ChoiceField(choices=[("python", "Python 3"), ("cpp", "C++17")], required=False, widget=forms.Select(attrs={"class": "form-select"}), label="Ngôn ngữ checker")

    class Meta:
        model = CodingExercise
        fields = ("title", "description", "is_enabled", "allowed_languages", "default_language", "time_limit_ms", "memory_limit_mb", "compare_mode", "checker_code", "checker_language")
        labels = {"compare_mode": "Checker", "time_limit_ms": "Time limit (ms)", "memory_limit_mb": "Memory limit (MB)", "checker_code": "Mã nguồn checker", "checker_language": "Ngôn ngữ checker"}
        widgets = {
            "description": forms.Textarea(attrs={"rows": 5, "class": "form-control", "placeholder": "Mô tả bài tập, yêu cầu input/output..."}),
            ...
            "checker_code": forms.Textarea(attrs={"rows": 10, "class": "form-control font-monospace", "placeholder": "Nhập mã nguồn checker tại đây..."}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["allowed_languages"].help_text = "Chỉ các ngôn ngữ được chọn mới hiện ra ở Monaco editor."
        self.fields["default_language"].choices = [("", "-- Chọn ngôn ngữ mặc định --")] + language_choices
        self.fields["title"].widget.attrs.update({"class": "form-control", "placeholder": "VD: In ra tổng 2 số"})
        ...
        for language_key, config in language_configs.items():
            ...
            self.fields[field_name] = forms.CharField(required=False, label=f"Starter code - {config.get('label', language_key)}", widget=forms.Textarea(attrs={"rows": 8, "class": "form-control font-monospace", "placeholder": "Code mẫu cho ngôn ngữ này..."}), initial=starter_map.get(language_key, config.get("starter_code", "")))
    ...
    def clean(self):
        ...
        if default_language and default_language not in allowed_languages:
            self.add_error("default_language", "Ngôn ngữ mặc định phải nằm trong danh sách cho phép.")
        if cleaned_data.get("is_enabled") and not allowed_languages:
            self.add_error("allowed_languages", "Cần chọn ít nhất một ngôn ngữ khi bật bài tập code.")
        if cleaned_data.get("compare_mode") == "custom_checker":
            if not cleaned_data.get("checker_code"):
                self.add_error("checker_code", "Vui lòng nhập mã nguồn checker.")
            if not cleaned_data.get("checker_language"):
...
class CodingTestCaseForm(forms.ModelForm):
    class Meta:
        ...
        fields = ("name", "input_text", "expected_output_text", "input_file", "expected_output_file", "is_sample", "order", "score", "subtask_id")
        widgets = {
            "name": forms.TextInput(attrs={"class": "form-control", "placeholder": "Ví dụ: sample-1, test-01"}),
            "input_text": forms.Textarea(attrs={"rows": 6, "class": "form-control font-monospace"}),
            "expected_output_text": forms.Textarea(attrs={"rows": 6, "class": "form-control font-monospace"}),
            ...
            "subtask_id": forms.NumberInput(attrs={"class": "form-control"}),
        }
    ...
    def clean(self):
        ...
        if not (cleaned_data.get("expected_output_text") or cleaned_data.get("expected_output_file")):
            self.add_error("expected_output_text", "Can output raw hoac file output.")
        return cleaned_data
...
class BaseChoiceFormSet(forms.models.BaseInlineFormSet):
    def clean(self):
        ...
        if any(self.errors):
            return
        completed_forms, correct_count = 0, 0
...
ChoiceFormSet = inlineformset_factory(Question, Choice, form=ChoiceForm, formset=BaseChoiceFormSet, extra=4, can_delete=True)
