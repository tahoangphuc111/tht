...
def can_publish_articles(user):
    ...
    return user.is_authenticated and (
        ...
        user.groups.filter(name__in=publisher_roles).exists() or
        user.has_perm('wiki.add_article')
    )
...
def build_profile_stats(user, viewer=None):
    ...
    articles = list(
        user.articles.select_related('category').order_by('-updated_at')
    )
    ...
    return {
        ...
        'edited_articles_count': ArticleRevision.objects.filter(
            author=user
